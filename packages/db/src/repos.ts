import type { Room } from "@rakazo/contracts";
import {
  type Actor,
  BOT_COLORS,
  type Bot,
...
export function createRepos(prisma: PrismaClient) {
  return {
    ...
    async listRooms(actor: Actor): Promise<Room[]> {
      const rooms = await prisma.thread.findMany({
        where: { workspaceId: actor.workspaceId, userId: actor.userId, kind: "room" },
        include: { participants: { orderBy: { addedAt: "asc" } } },
        orderBy: { createdAt: "desc" },
      });
      return rooms.map(mapRoom);
    },

    async getRoom(actor: Actor, roomId: string): Promise<Room> {
      const room = await prisma.thread.findFirst({
        where: {
          id: roomId,
          workspaceId: actor.workspaceId,
          userId: actor.userId,
    ...
    async createRoom(actor: Actor, input: { name: string; botIds: string[] }): Promise<Room> {
      const botIds = [...new Set(input.botIds)];
      // Every participant must be a live bot in this workspace, so a room can
      // never be used to reach across tenants.
      const bots = await prisma.bot.findMany({
        where: {
          id: { in: botIds },
          workspaceId: actor.workspaceId,
      ...
      const room = await prisma.thread.create({
        data: {
          ...
          participants: { create: bots.map((bot) => ({ botId: bot.id })) },
        },
        include: { participants: { orderBy: { addedAt: "asc" } } },
      });
      return mapRoom(room);
    ...
    async setRoomParticipant(
      actor: Actor,
      input: { roomId: string; botId: string; member: boolean },
    ): Promise<Room> {
      const room = await prisma.thread.findFirst({
        where: {
          id: input.roomId,
      ...
      if (input.member) {
        await prisma.threadParticipant.upsert({
          where: { threadId_botId: { threadId: room.id, botId: bot.id } },
          create: { threadId: room.id, botId: bot.id },
          update: {},
        });
      } else {
        await prisma.threadParticipant.deleteMany({
          where: { threadId: room.id, botId: bot.id },
        });
      }
      const updated = await prisma.thread.findUniqueOrThrow({
        where: { id: room.id },
        include: { participants: { orderBy: { addedAt: "asc" } } },
      });
    ...
    async deleteRoom(actor: Actor, roomId: string): Promise<void> {
      ...
      if (deleted.count === 0) throw new IsolationError();
    },
  };
}
...
function mapRoom(room: {
  ...
  name: string | null;
  createdAt: Date;
  participants: { botId: string }[];
}): Room {
  return {
    id: room.id,
    workspaceId: room.workspaceId,
    name: room.name ?? "Room",
    botIds: room.participants.map((participant) => participant.botId),
