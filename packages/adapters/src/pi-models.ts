...
function catalogBilling(
  ...
  if (providerId === LOCAL_PROVIDER_ID) {
    return "Runs on infrastructure configured by the deployment owner. No model charges from this deployment.";
  }
  if (providerId === OPENAI_COMPATIBLE_PROVIDER_ID) {
    return "Runs on a URL you control. Model usage is billed to your own endpoint.";
  }
  if (opts.oauth && !opts.apiKey) {
    return `${name} subscription login is not in this UI yet. Skip if this deployment already has credentials.`;
  }
  if (opts.apiKey) {
    return `Uses your ${name} API key. Model usage is billed to your ${name} account.`;
  }
  return `Uses your ${name} key. Model usage is billed to your ${name} account.`;
}
