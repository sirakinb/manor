...
function desktop(platform: string): RakazoDesktop {
  ...
  return {
    ...
    localComputer: {
      ...
      connect: async () => ({ sharing: false, folderName: null, lastCommand: null }),
      stop: async () => ({ sharing: false, folderName: null, lastCommand: null }),
      status: async () => ({ sharing: false, folderName: null, lastCommand: null }),
      onChange: () => () => undefined,
    },
  };
}
...
describe("window chrome", () => {
  ...
  it("keeps conversation header controls clickable", () => {
    ...
    // Every header control, including Files and Run logs, must remain clickable.
    const controls = shell.match(/className="[^"]*grid h-\[30px\] w-\[34px\][^"]*"/g) ?? [];
    expect(controls.length).toBeGreaterThan(0);
    for (const control of controls) expect(control).toContain("app-no-drag");
  });
});
