...
8. Put the web app behind the same origin as `/api`, `/rpc`, `/v1`, `/mcp`, and `/local-computer` (Vite preview proxy, or a reverse proxy). `/local-computer` must reach the API, not website HTML. Docker noVNC connections use short-lived signed `/novnc/*` capabilities; do not replace that route with an unrestricted port proxy.
