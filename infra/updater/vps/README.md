...
health probe and maintenance adapter. It requires `ownerTokenFile`,
`developerTokenFile` and `maintenanceTokenFile`; all three must be regular,
operator-owned files with no group or world permissions (mode 0600).
Private executor diagnostics are retained in `last-error.txt`, including
failures before an operation starts. The CLI `history` command returns
operation history, matching the HTTP endpoint. Both Compose and environment files
